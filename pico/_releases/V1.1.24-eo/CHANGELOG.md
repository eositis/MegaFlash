...
**Firmware version:** `0x001c` — string `V1.1.24-eo`

## What’s new in V1.1.24-eo

- **Firmware build string left of clock:** The Control Panel again polls `CMD_GETFIRMWAREVER` (0x29) in `DisplayTime` and draws the 12-character build string at columns 20–31, with the live time at 32–39. `ClearTime` clears columns 20–39 so the format flow does not leave stale version text. `cpanel/asm-megaflash.s`.
- **Drives Enable — checkbox toggle row:** cc65 `gotoxy(x, y)` uses **window-relative** `y` (cursor row is `WNDTOP + y`). Toggle redraws were passing `YPOS + driveRow`, which placed updates about **one window top** too low (e.g. 6 lines when `YPOS == WNDTOP`). Fixed by passing relative rows only (`i`, `listCount`, and `listCount + 1` for the ROM disk line). The ROM disk text row and checkbox use `romdiskRow = listCount + 1`. `cpanel/drivesenable.c`.

## UF2 images in this folder

| File | Board |
|------|--------|
| `megaflash-pico.uf2` | Raspberry Pi Pico W (RP2040 + CYW43 Wi‑Fi) |
| `megaflash-pico2.uf2` | Raspberry Pi Pico 2 W (RP2350 + CYW43 Wi‑Fi) |

Flash the appropriate UF2 in BOOTSEL mode; MegaFlash remains in slot 4 as usual.

---

## Included changes from V1.1.23-eo

The following items were part of **V1.1.23-eo** and remain in this build.
...
- **Build ID left of clock (API):** `CMD_GETFIRMWAREVER` (0x29) polls firmware version separately (ProDOS-safe). `pico/cmdhandler.c`, `common/defines.h`.
