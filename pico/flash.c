...
static uint32_t __no_inline_not_in_flash_func(tsReadSector)(const uint deviceNum,uint32_t sectorAddress){
  ...
  enable_spi0(deviceNum);
  spi_write_blocking(spi0, msg, 6);
  uint32_t crc=ReadFromFlashByDMA(sectorBuffer,SECTORSIZE,&success);
  disable_spi0();
  ...
  if (!success) {
    enable_spi0(deviceNum);
    spi_write_blocking(spi0, msg, 6);
    spi_read_blocking(spi0, REPEATED_TX_DATA, sectorBuffer, SECTORSIZE);
    disable_spi0();
    crc = CRC32Aligned(sectorBuffer,SECTORSIZE);
  }
  MUTEXUNLOCK();
