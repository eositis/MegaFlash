...
/* WGET65V (MegaFlash): Uthernet II is fixed at slot 4 → $C0C4–$C0C7 (ip65 a2init). */
#define WGET65V_ETHER_SLOT 4
...
int main(int, char *argv[])
{
  ...
  char device;
  bool Offload_DNS;
  uint8_t eth_init = WGET65V_ETHER_SLOT;

#ifdef __APPLE2__
  /* 80 columns for verbose handshake lines (same as ip65 hfs65.c). */
  ...
  printf("\nWGET65V: slot %u — Uthernet II at $C0C4-$C0C7 (not using ethernet.slot)\n",
         (unsigned)eth_init);

  wget65_verbose_banner();
  wget65_verbose_stepf("Ethernet slot (fixed) = %u — mode reg $C0C4", (unsigned)eth_init);

  wget65_verbose_step("Step A: ip65_init() — loads IP65; W5100 driver probes chip (RTR XOR)");
  printf("- %u\n\nInitializing %s ", eth_init, eth_name);
  if (ip65_init(eth_init))
  {
    wget65_verbose_step("Step A FAILED — ip65_init returned error (often W5100 not found)");
    printf("[WGET65V] ip65_error=%u ($%02X) — %s\n", (unsigned)ip65_error, (unsigned)ip65_error,
           ip65_strerror(ip65_error));
    printf(
      "[WGET65V] Note: dump below runs AFTER eth_init. MR/RTR can still look\n"
      "  correct: w5100.s RTR XOR runs earlier; PIO/bus timing may differ from\n"
      "  these reads (MegaFlash docs Implementation-notes §1c / §1d).\n\n");
    wget65_dump_w5100_regs(eth_init, "after failed ip65_init");
    ip65_error_exit();
  }
  wget65_verbose_step("Step A OK — ip65_init cleared");
  wget65_dump_w5100_regs(eth_init, "after ip65_init");

  /* Abort on Ctrl-C to be consistent with Linenoise */
  abort_key = 0x83;

  wget65_verbose_step("Step B: w5100_init() — map ports; DNS offload if PTIMER=$00");
  Offload_DNS = w5100_init(eth_init);
  wget65_verbose_stepf("Step B result: DNS offload = %s", Offload_DNS ? "yes" : "no");
  wget65_dump_w5100_regs(eth_init, "after w5100_init");

  if (!Offload_DNS)
  {
    wget65_verbose_step("Step C: dhcp_init()");
    printf("- Ok\n\nObtaining IP address ");
    if (dhcp_init())
    {
      wget65_verbose_step("Step C FAILED — dhcp_init");
      wget65_dump_w5100_regs(eth_init, "after failed dhcp_init");
      ip65_error_exit();
    }
    wget65_verbose_step("Step C OK — dhcp_init");
  }
  else
  {
    wget65_verbose_step("Step C skipped — using virtual W5100 DNS offload");
  }
  printf("- Ok\n\n");

  wget65_verbose_step("Step D: w5100_config() — write SIP/mask/gateway; socket memory");
  /* Copy IP config from IP65 to W5100 */
  w5100_config();
  wget65_dump_w5100_regs(eth_init, "after w5100_config");
  wget65_verbose_step("End handshake trace — entering URL / file prompts\n");
